...
def detect_edges1(frame):
    ...
    return edges
def detect_edges2(frame):
    gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
    kernel = 5
    blur = cv2.GaussianBlur(gray, (kernel, kernel), 0)

    edges = cv2.Canny(gray, 50, 150)  # a=50 b= 150
...
def show_image(title, frame, show=SHOW):
    if show:
